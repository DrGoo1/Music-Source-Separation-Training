...
def build_midi_database(midi_root):
    """
    Recursively scan the MIDI root directory for MIDI files that follow the naming convention:
      <id>_<style>_<bpm>_beat_<time_signature>_<version>.midi
    Returns a dictionary mapping the base filename (without extension) to a metadata dict.
    """
    midi_db = {}
    for dirpath, _, filenames in os.walk(midi_root):
        for file in filenames:
            if file.lower().endswith(('.mid', '.midi')):
                ...
                parts = base.split('_')
                if len(parts) >= 5:
                    # Extract style, BPM, and time signature from the filename.
                    style = parts[1]
                    try:
                        bpm = int(parts[2])
                    except ValueError:
...
def build_audio_database(audio_root):
    ...
    Returns a dictionary mapping the base filename (without extension) to the full file path.
    """
    audio_db = {}
...
def organize_by_style(combined_db):
    ...
    Here, the category is derived by splitting the style string at '-' and capitalizing the first part.
    """
    organized = {}
...
def main():
    st.title("Combined E-GMD Database Viewer")
    st.write("""
        This tool scans the E‑GMD database for MIDI and corresponding audio files, then organizes them by drumming style.
        You can browse the styles and view details (including file paths) for each entry.
    """)
    ...
    if st.button("Build Combined Database"):
        ...
        # Allow the user to select a style category.
        categories = list(organized_db.keys())
        selected_category = st.selectbox("Select Category:", categories)
        styles_in_category = organized_db[selected_category]
        selected_entry_key = st.selectbox("Select an entry from " + selected_category, list(styles_in_category.keys()))
        selected_entry = styles_in_category[selected_entry_key]
        st.write(f"Details for {selected_entry_key}:")
        st.json(selected_entry)

        # Optionally, play the audio snippet.
